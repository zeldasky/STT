"""
Mac용 한국어 음성-텍스트 변환 시스템
- 화자 분리 (Speaker Diarization)
- 한국어 STT (Speech-to-Text)
- 회의록 자동 생성
"""
...
class KoreanSTTSystem:
    ...
    def __init__(self, model_name="kresnik/wav2vec2-large-xlsr-korean", huggingface_token=None):
        """
        Args:
            model_name: HuggingFace 모델명
            huggingface_token: HuggingFace API 토큰 (화자 분리 시 필요)
        """
        print("🔄 모델 로딩 중...")

        # Mac에서 MPS(Metal Performance Shaders) 사용 가능 여부 확인
    ...
    def load_audio(self, audio_path, target_sr=16000):
        ...
        Args:
            ...
            target_sr: 목표 샘플링 레이트 (wav2vec2는 16kHz 필요)
    ...
    def transcribe_with_speakers(self, audio_path):
        ...
        Returns:
            list: [{'speaker': str, 'start': float, 'end': float, 'text': str}, ...]
        """
        # 오디오 로드
    ...
    def save_results(self, results, output_dir="output"):
        ...
        Args:
            results: transcribe_with_speakers 결과
            output_dir: 출력 디렉토리
        """
        # 출력 디렉토리 생성
...
def main():
    ...
    print("=" * 80)
    print("🎙️ Mac용 한국어 음성-텍스트 변환 시스템")
    print("=" * 80)
    print()

    # HuggingFace 토큰 (환경변수 또는 직접 입력)
    hf_token = os.getenv("HUGGINGFACE_TOKEN")  # 환경변수에서 읽기
    ...
    # Mac에서 드래그 앤 드롭 시 경로에 이스케이프 문자 제거
